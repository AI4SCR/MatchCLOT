...
class Encoder(nn.Module):
    ...
    def __init__(self, n_input, embedding_size, dropout_rates, dims_layers, swap_noise_ratio, noise_amount=0.0):
    ...
    def forward(self, x):
        for i in range(len(self.fc_list) - 1):
            if i > 0 and self.training and self.noise_amount > 0:
                x = torch.mul(x, torch.ones_like(x) + self.noise_amount * torch.randn_like(x, device=x.device))
...
class Modality_CLIP(nn.Module):
    ...
    def __init__(self, Encoder, layers_dims, dropout_rates, dim_mod1, dim_mod2, output_dim, T, swap_rate_1=0.0,
                 swap_rate_2=0.0, noise_amount=0.0, ):
