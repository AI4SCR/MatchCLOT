...
def get_dataloaders(mod1_train, mod2_train, sol_train, mod1_test, mod2_test, sol_test, NUM_WORKERS, BATCH_SIZE):
...
class AugDataset(Dataset):
    ...
    def __init__(
            self, df_modality1, df_modality2, transf_matrices_mod1, transf_vectors_mod1, transf_matrices_mod2,
            transf_vectors_mod2
    ):
        """
        Args:
        ...
        T = torch.Tensor([[rfirst[col][row] - r4[row] for col in range(n_dim)] for row in range(n_dim)])
    ...
    def __getitem__(self, index: int):
        ...
        x_mod1 = aug_strength * (
                c1 * (self.transf_matrices_mod1[0] @ x_mod1 + self.transf_vectors_mod1[0])
                + c2 * (self.transf_matrices_mod1[1] @ x_mod1 + self.transf_vectors_mod1[1])
                + c3 * (self.transf_matrices_mod1[2] @ x_mod1 + self.transf_vectors_mod1[2])
                + c4 * (self.transf_matrices_mod1[3] @ x_mod1 + self.transf_vectors_mod1[3])
        ) + (1 - aug_strength) * x_mod1
        x_mod2 = aug_strength * (
                c1 * (self.transf_matrices_mod2[0] @ x_mod2 + self.transf_vectors_mod2[0])
                + c2 * (self.transf_matrices_mod2[1] @ x_mod2 + self.transf_vectors_mod2[1])
                + c3 * (self.transf_matrices_mod2[2] @ x_mod2 + self.transf_vectors_mod2[2])
                + c4 * (self.transf_matrices_mod2[3] @ x_mod2 + self.transf_vectors_mod2[3])
...
class SingleAugDataset(Dataset):
    ...
    def __init__(
            self, df_modality1, df_modality2, transf_matrix_mod1, transf_vector_mod1, transf_matrix_mod2,
            transf_vector_mod2
    ):
        """
        Args:
    ...
    def __getitem__(self, index: int):
        ...
        aug_strength = torch.rand(size=(1,))
        # use the same coefficients for the two modalities so the matching still makes sense
        x_mod1 = aug_strength * (self.transf_matrix_mod1 @ x_mod1 + self.transf_vector_mod1) \
                 + (1 - aug_strength) * x_mod1
        x_mod2 = aug_strength * (self.transf_matrix_mod2 @ x_mod2 + self.transf_vector_mod2) \
                 + (1 - aug_strength) * x_mod2
