...
def MWB_matching(raw_logits, threshold_quantile=0.99):
    while threshold_quantile >= 0:
        try:
            ...
            del weights
            graph = networkx.algorithms.bipartite.matrix.from_biadjacency_matrix(weights_sparse)
            u = [n for n in graph.nodes if graph.nodes[n]["bipartite"] == 0]
            matches = networkx.algorithms.bipartite.matching.minimum_weight_full_matching(graph, top_nodes=u)
            ...
            print("Discarding too many edges, problem infeasible, lowering threshold to {}".format(threshold_quantile))
